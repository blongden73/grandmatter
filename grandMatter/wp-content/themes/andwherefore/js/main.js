    ...
    if(image) {
        document.addEventListener('scroll', function(){
            for(i=0; i<image.length; i++) {
                console.log(elementInViewport(image[i]));
                if(elementInViewport(image[i])){
                    var data = image[i].dataset.url;
                    if(image[i].innerHTML.indexOf('fade-in-image') < 1) {
                        image[i].innerHTML = '<img class="fade-in-image" src="' + data + '">';
                    }
                }
            }
        })
...
function galleryNumbers() {
    var numbers = document.getElementsByClassName('aw__gallery-count');
    if(numbers){
        for(i=0; i< numbers.length; i++){
            numbers[i].innerHTML = i+1;
        }
    }
    var galleryIndex = document.getElementsByClassName('aw__gallery-index');
    if(galleryIndex){
        for(i=0; i< numbers.length; i++){
            galleryIndex[i].innerHTML = i+1+' ';
        }
    }
}

function andWhereforeTop(){
    var image = document.querySelector('.aw__top-image');
    if(image){
        var images = document.getElementsByClassName('aw__splash-image');
        console.log(images);
        var anim = setInterval(function(){ slides() }, 1000);
        var count = 0;
        function slides(){
            console.log('here');
